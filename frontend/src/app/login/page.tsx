...
export default function LoginPage() {
  ...
  const handleSelectGoogleAccount = async (accountEmail: string, accountName: string) => {
    ...
    setIsSubmitting(true);
    try {
      const ok = await loginWithGoogle(accountEmail, accountName);
      if (ok) {
        setIsGoogleModalOpen(false);
        setSuccessMessage(`Signed in with Google as ${accountName} (${accountEmail})`);
        setTimeout(() => router.push("/profile"), 600);
      } else {
        ...
        <div className="rounded-3xl border-2 border-[#e8dede] p-6 sm:p-8 bg-[#ffffff] shadow-[0_16px_48px_rgba(177,133,151,0.12)] space-y-6 relative overflow-hidden">
          ...
          <div className="space-y-3 relative z-10">
            <button
              ...
              className="w-full py-3.5 px-4 rounded-2xl bg-[#ffffff] hover:bg-[#faf6f6] border-2 border-[#e8dede] hover:border-[#b18597] text-[#2b201a] font-semibold text-xs uppercase tracking-wider flex items-center justify-center gap-3 shadow-[0_4px_0_0_#e8dede] hover:shadow-[0_4px_0_0_#b18597] active:translate-y-1 transition-all cursor-pointer group"
            >
              {/* Google G Logo SVG */}
